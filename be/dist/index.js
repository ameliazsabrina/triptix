...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, user) => {
        ...
        const isBlacklisted = await pool.query("SELECT token FROM triptix_db.token_blacklist WHERE token = $1", [token]);
...
app.post("/api/register", async (req, res) => {
    ...
    try {
        ...
        const userExists = await pool.query("SELECT * FROM triptix_db.user WHERE email = $1 OR username = $2", [email, username]);
...
app.post("/api/logout", authenticateToken, async (req, res) => {
    ...
    try {
        await pool.query("INSERT INTO triptix_db.token_blacklist (token) VALUES ($1)", [token]);
...
app.post("/api/login", async (req, res) => {
    ...
    try {
        ...
        const user = await pool.query("SELECT * FROM triptix_db.user WHERE username = $1", [username]);
...
app.get("/api/user", authenticateToken, async (req, res) => {
    try {
        ...
        const userResult = await pool.query("SELECT user_id, username, email FROM triptix_db.user WHERE user_id = $1", [req.user.userId]);
...
app.post("/api/generate-plan", authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const createTrip = await pool.query(`INSERT INTO triptix_db.trip (user_id, location, start_date, end_date, budget, travel_partner, generate_result, is_saved)
    ...
    `;
        ...
        const updateTrip = await pool.query(`UPDATE triptix_db.trip
       SET generate_result = $1
       WHERE id = $2
       RETURNING *`, [JSON.stringify(formattedResponse), createTrip.rows[0].id]);
        res.status(200).json({
            success: true,
...
app.put("/api/saved-plans/:id", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const result = await pool.query(`UPDATE triptix_db.trip 
       SET is_saved = TRUE 
...
app.get("/api/trips", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const result = await pool.query("SELECT * FROM triptix_db.trip WHERE user_id = $1 AND is_saved = TRUE", [userId]);
        res.json({
            success: true,
            trips: result.rows,
        });
...
app.delete("/api/saved-plans/:id", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const result = await pool.query("DELETE FROM triptix_db.trip WHERE id = $1 AND user_id = $2", [id, userId]);
...
app.get("/api/trips/:id", authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const result = await pool.query("SELECT * FROM triptix_db.trip WHERE id = $1 AND user_id = $2", [id, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
